...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const messageTextContent: Array< MessageContentText> = threadMessages.data[0].content.filter(item => item.type === 'text')
    const messageImageContent: Array<MessageContentImageFile> = threadMessages.data[0].content.filter(item => item.type === 'image_file')

    // console.log(fileIds)
    // .content.find(item => item.type === 'image_file')
    // messageFilesId
    ...
    return {
      ...
      attachments: messageImageContent,
    }
  }
  catch (error) {
    throw createError({
